...
- (void)login:(id)requestDelegate requestSelector:(SEL)requestSelector andErrorSelector:(SEL)errorSelector {
	isPost = NO;
...
-(void)statuses_update:(NSString *)status delegate:(id)requestDelegate requestSelector:(SEL)requestSelector andErrorSelector:(SEL)errorSelector {
	isPost = YES;
	// Set the delegate and selector
	self.delegate = requestDelegate;
	self.callback = requestSelector;
...
//		[newCredential release];
		...
		DLog(@"Invalid Username or Password");
		// !!!!IF THERE IS AN ERROR RESET PASSWORD!!!
    }
...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection {
	...
	if(delegate && callback) {
		if([delegate respondsToSelector:self.callback]) {
			...
			DLog(@"No response from delegate");
		}
	} 
